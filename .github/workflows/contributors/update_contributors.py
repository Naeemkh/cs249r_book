...
EXCLUDED_USERS = {"web-flow", "github-actions[bot]", "mrdragonbear", "jveejay", "Matthew Steward"}
...
def get_user_data_from_username(username):
    ...
    if res.status_code == 200:
        user_data = res.json()
        user_full_name = user_data.get('name', pd.NA)
        email_address = user_data.get('email', pd.NA)
    else:
        logging.error(f'Could not find user with username: {username}')
    return {'username': username, 'user_full_name': user_full_name, 'email_address': email_address}

def get_user_data_from_email(email_address):
    headers = {"Authorization": f"token {os.environ['GITHUB_TOKEN']}"}
    res = requests.get(f"https://api.github.com/search/users?q={email_address}", headers=headers)
    username = pd.NA
    if res.status_code == 200:
        user_data = res.json()
    ...
    return {'username': username, 'user_full_name': pd.NA, 'email_address': email_address}
    ...
    if commit_message:
        ...
        for line in lines:
            try:
                ...
                logging.error(f"Error parsing co-author: {line}. Co-author should be of the form: 'Co-authored-by: NAME <email>'. Remember to include the angle brackets around the email.")
...
def main(_):
    ...
    data = []
    next_page = f"https://api.github.com/repos/{OWNER}/{REPO}/commits?sha={BRANCH}&per_page=500"
    last_page = None
    ...
    for node in data:
        ...
        commit_data.append({"commit_message": commit_message, "user_full_name": user_full_name, "username": username})
    commit_data_df = pd.DataFrame(commit_data)
    co_authors_list = [get_co_authors_from_commit_message(row["commit_message"]) for index, row in commit_data_df.iterrows()]
    ...
    commit_data_df = commit_data_df.merge(co_authors_df, how='outer', on=['user_full_name'])

    # Remove rows where the username or user_full_name is in the EXCLUDED_USERS list in one line
    commit_data_df = commit_data_df[
        ~commit_data_df['username'].isin(EXCLUDED_USERS) & ~commit_data_df['user_full_name'].isin(EXCLUDED_USERS)
    ]

    # Before we drop duplicates, get the number of commits per user
    commit_data_df = commit_data_df.assign(commits=commit_data_df['user_full_name'].map(commit_data_df['user_full_name'].value_counts()))
    ...
    for index, row in commit_data_df.iterrows():
        if not pd.isna(row.username):
            ...
            commit_data_df.loc[index, 'user_full_name'] = user_data['user_full_name']
    ...
    commit_data_df = commit_data_df.assign(name_length=commit_data_df['user_full_name'].str.len())
    commit_data_df = commit_data_df.fillna(pd.NA)
    commit_data_df = commit_data_df.sort_values(by=['commits', 'name_length'], ascending=False)
    ...
    commit_data_df = commit_data_df.groupby(['has_username', 'username', 'email_address'], dropna=False, as_index=False).first()
    ...
    commit_data_df['user_full_name'] = commit_data_df.apply(
        lambda row: row['username'] if pd.isna(row['user_full_name']) else row['user_full_name'], axis=1
    )
    ...
    commit_data_df['avatar_url'] = commit_data_df.apply(
        lambda row: generate_gravatar_url(row['user_full_name']) if pd.isna(row['username']) else f"https://avatars.githubusercontent.com/{row['username']}", axis=1
    )

    # Update profile URL
    commit_data_df['profile'] = commit_data_df.apply(
        lambda row: "https://github.com/harvard-edge/cs249r_book/graphs/contributors" if pd.isna(row['username']) else f"https://github.com/{row['username']}", axis=1
    )
    ...
    final_result = dict(
        ...
        contributors=[
            dict(
                login=row.username,
                name=row.user_full_name if not pd.isna(row.user_full_name) else row.username,
